        ...
        <div className="relative mt-8 sm:mt-9">
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6 md:gap-6 lg:gap-4 xl:gap-6">
            ...
            {categories.map((category, index) => (
              <div key={index} className="relative z-0 h-40 overflow-hidden transition duration-300 ease-in-out transform translate-y-0 shadow-md cursor-pointer rounded-2xl group hover:-translate-y-1">
                <Link href={`/categories/${category.frontmatter.name.replace(/ /g, '-').toLowerCase()}`}>
                  <a className="absolute inset-0 z-10 w-full h-full shadow-md rounded-2xl" />
                </Link>
